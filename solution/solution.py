...
Hướng dẫn:
    ...
    4. Import Gemini BÊN TRONG hàm để các bài test mock không gọi API thật.
...
# Tên model có thể đổi qua .env. Không đặt gì thì dùng Gemini mặc định.
GEMINI_MODEL = os.getenv("LAB_MODEL", "gemini-2.5-flash")
GEMINI_MINI_MODEL = os.getenv("LAB_MINI_MODEL", "gemini-3.5-flash-lite")
...
def call_openai(
    ...
    """
    Gọi Gemini API, trả về nội dung phản hồi + độ trễ.
...
def call_openai_mini(
    ...
    """
    Gọi API với model Gemini nhẹ — nhanh hơn và rẻ hơn.

    Returns:
        Tuple (response_text: str, latency_seconds: float).

    Gợi ý:
        Tái sử dụng call_openai() với model=GEMINI_MINI_MODEL — 1 dòng code.
    """
    return call_openai(
        prompt=prompt,
...
def compare_models(prompt: str) -> dict:
    ...
    ) * PRICING_PER_1K_TOKENS.get(GEMINI_MODEL, PRICING_PER_1K_TOKENS["gemini-2.5-flash"])["output"]
...
def count_tokens(text: str, model: str = GEMINI_MODEL) -> int:
    ...
    try:
        ...
        # Fallback: trung bình khoảng 1 token ≈ 4 ký tự
...
def estimate_cost(prompt: str, response: str, model: str = GEMINI_MODEL) -> dict:
    ...
    pricing = PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gemini-2.5-flash"])
    input_cost = input_tokens / 1000 * pricing["input"]
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        stream = client.models.generate_content_stream(model=GEMINI_MODEL, contents=history)
        reply_parts = []
        for chunk in stream:
            text = chunk.text or ""
        ...
        print()
        history.append({"role": "model", "parts": [{"text": "".join(reply_parts)}]})
        history = history[-6:]
...
def run_assistant(
    ...
    while max_turns is None or num_turns < max_turns:
        ...
        stream = retry_with_backoff(lambda: client.models.generate_content_stream(
            model=GEMINI_MODEL,
            contents=contents,
        ...
        history = history[-6:]
        cost = estimate_cost(user_message, reply, GEMINI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        rows.append([
            result.get("prompt", "")[:40], result.get("gpt4o_response", "")[:40],
            result.get("mini_response", "")[:40],
            f"{result.get('gpt4o_latency', 0):.3f}s", f"{result.get('mini_latency', 0):.3f}s",
        ])
