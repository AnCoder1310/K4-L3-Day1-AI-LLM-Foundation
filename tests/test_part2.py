...
class TestChatWithSystemPrompt(unittest.TestCase):
    ...
    @patch("google.genai.Client")
    def test_messages_contain_system_and_user_roles(self, MockClient):
        ...
        # Gemini truyền user prompt qua contents
        self.assertEqual(
            kwargs.get("contents"),
            user_prompt
        )

        # System prompt nằm trong config
...
class TestEstimateCost(unittest.TestCase):
    ...
    PROMPT = (
        "Hãy kể cho tôi một sự thật thú vị về Việt Nam. " * 5
    )

    RESPONSE = (
        "Việt Nam là nước xuất khẩu cà phê lớn thứ hai thế giới. " * 5
    )
    ...
    def test_mini_is_cheaper_than_flash(self):

        cost_flash = MOD.estimate_cost(
            self.PROMPT,
            self.RESPONSE,
            model="gemini-2.5-flash"
        )

        cost_mini = MOD.estimate_cost(
            self.PROMPT,
            self.RESPONSE,
